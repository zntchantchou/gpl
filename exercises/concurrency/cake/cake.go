...
// only takes a channel that receives cakes
func (s *Shop) baker(baked chan<- cake) {
	...
	// baked is full at this point, we know its capacity form the start which is s.Cakes
	close(baked)
}
...
func (s *Shop) Work(runs int) {
	for run := 0; run < runs; run++ {
		...
		// there is no inscribed buffer because inscribing is the last operation
		go s.baker(baked)
